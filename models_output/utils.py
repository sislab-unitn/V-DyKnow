...
EXCEPTIONS = {
    "athletes": {
        ...
        "Kylian Mbapp\u00e9": [
            "France national under-17 association football team",
            "France national under-19 association football team"
        ],
        "Karim Benzema": [
            "France national under-17 association football team",
            "France national under-21 association football team"
        ],
        "Sadio Man\u00e9": [
        ...
        "Harry Kane": [
            ...
            "England national under-19 association football team",
...
ADDITIONAL_BITS = {
    "athletes": [
        r"[\w]?F(\.)?C(\.)?[\w]?",  # FC, F.C., AFC, ... for Football
        r"[\w]?C(\.)?F(\.)?[\w]?",  # CF, ... for Football
        r"[\w]?F(\.)?K(\.)?[\w]?",  # FK, ... for Football
        r"[\w]?A(\.)?S(\.)?[\w]?",  # AS, ... for Football
        r"[\w]?S(\.)?V(\.)?[\w]?",  # SV, ... for Football
        r"[\w]?B(\.)?C(\.)?[\w]?",  # BC, ... for Basketball
        r"[\w](\.)[\w](\.)",  # General regex for to remove two letter acronyms (with )
        r"football",
        r"(t|T)eam",
        r"association",
        r"men's",
